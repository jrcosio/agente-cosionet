...
#
# Corre `bot_telegram.py`, y eso decide todo lo demás: el bot funciona por
...
#   · no necesita dominio
#   · no necesita health check (y si el panel te lo enciende solo, apágalo)
#
...
# es que con polling no hay a quién pegarle. Bórrale el dominio y deja el
# health check apagado.
#
# Toda la configuración entra por variables de entorno. El .env no se copia
# aquí adentro: en el servidor las variables las pone el panel.
#
...
#   · **Una sola instancia a la vez.** Si el bot queda corriendo aquí y también
#     en tu ordenador, los dos le piden a Telegram los mismos mensajes y se
#     los reparten al azar: la mitad de las respuestas sale de cada lado. Es
#     la falla más confusa de todas, porque parece que funciona a veces sí y a
#     veces no.
#   · `PROVEEDOR=chatgpt` **no sirve aquí.** La sesión de ChatGPT vive en tu
...
# instala exactamente las mismas versiones que tienes en tu máquina.
...
# Tres cosas que le pedimos a uv aquí adentro:
#
...
#   · no descargar ningún Python: con el de esta imagen basta. Si algo no
...
#             quedó viejo** respecto del pyproject. Es lo que quieres en un
#             servidor: que avise aquí y no que resuelva versiones nuevas por
...
#
# No copiamos el `.python-version`: aquí la versión de Python la fija la
